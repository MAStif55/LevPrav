...
export default function Home() {
    ...
    const { products, fetchProducts } = useProductCache();

    // Prefetch products on homepage load - this warms up the cache
    useEffect(() => {
        fetchProducts();
    }, [fetchProducts]);

    // Show first 4 products from cache (or skeletons while loading)
    const featuredPacks = products.slice(0, 4);
    const showSkeletons = products.length === 0;

    return (
        <PublicLayout>
            <Head>
                <title>ЛевПрав | Арт-студия</title>
                <meta name="description" content="Арт-студия авторских handmade изделий. The Lion is Right." />
            </Head>
                ...
                <section className="py-24 bg-gray-50">
                    <div className="container mx-auto px-4">
                        <div className="flex justify-between items-end mb-12">
                            ...
                            <Link href="/packs" className="hidden md:flex items-center text-forest-green font-bold hover:underline">
                        ...
                        <div className="mt-12 text-center md:hidden">
                            <Link href="/packs" className="inline-block border border-graphite text-graphite px-8 py-3 rounded-full font-bold hover:bg-graphite hover:text-white transition-colors">
